...
Select Payment_method, count(*) as No_payments, sum(Quantity) as No_of_Qty_sold
from walmart 
...
SELECT * FROM (
    SELECT Branch, Category, AVG(Rating) AS Avg_Rating,
           RANK() OVER (PARTITION BY Branch ORDER BY AVG(Rating) DESC) AS Col
    FROM walmart 
...
FROM (
    SELECT 
        ...
        RANK() OVER(PARTITION BY branch ORDER BY COUNT(*) DESC) AS rank_col
    FROM walmart
...
WITH cte AS (
    SELECT 
        ...
        RANK() OVER(PARTITION BY branch ORDER BY COUNT(*) DESC) AS col
    FROM walmart
